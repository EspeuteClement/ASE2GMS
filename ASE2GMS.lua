...
local function skip_delim(str, pos, delim, err_if_missing)
  ...
  if str:sub(pos, pos) ~= delim then
    if err_if_missing then
      local_assert(false, 'Expected ' .. delim .. ' near position ' .. pos)
    end
    return pos, false
  end
  return pos + 1, true
...
local function parse_str_val(str, pos, val)
  ...
  if pos > #str then local_assert(false, early_end_error) end
  ...
  if not nextc then local_assert(false, early_end_error) end
...
local function parse_num_val(str, pos)
  ...
  if not val then local_assert(false, 'Error parsing number at position ' .. pos .. '.') end
...
function json.stringify(obj, as_key)
  ...
  if kind == 'array' then
    if as_key then local_assert(false, 'Can\'t encode array as key.') end
    s[#s + 1] = '['
  ...
  elseif kind == 'table' then
    if as_key then local_assert(false, 'Can\'t encode table as key.') end
    s[#s + 1] = '{'
  ...
  elseif kind == 'nil' then
    ...
    local_assert(false, 'Unjsonifiable type: ' .. kind .. '.')
  end
...
function json.parse(str, pos, end_delim)
  pos = pos or 1
  if pos > #str then local_assert(false, 'Reached unexpected end of input.') end
  ...
  if first == '{' then  -- Parse an object.
    ...
    while true do
      ...
      if not delim_found then local_assert(false, 'Comma missing between object items.') end
  ...
  elseif first == '[' then  -- Parse an array.
    ...
    while true do
      ...
      if not delim_found then local_assert(false, 'Comma missing between array items.') end
      arr[#arr + 1] = val
      pos, delim_found = skip_delim(str, pos, ',')
    end
  ...
  elseif first == end_delim then  -- End of an object or array.
    ...
    local_assert(false, 'Invalid json syntax starting at ' .. pos_info_str)
  end
end
...
local function local_assert(cond, msg)
	if (not cond) then
		print(msg)
		error()
	end
...
function Main()
	...
	function WindowsPathEscape(str)
		local_assert(IsValidPath(str), string.format("Error, invalid path %s", str))
		return '"'..str:gsub("/","\\")..'"'
	end
	...
	function ExportTag(spriteToExport, from, to, exportName)
		...
		-- backup previous sprite properties
		local prevFile = io.open(spriteFilepath, "r");

		local parentName = "Sprites";
		local parentPath = "folders/Sprites.yy"
		
		local texturegroupname = "Default"
		local texturegrouppath = "texturegroups/Default"

		if prevFile then
			local prevFileTxt = prevFile:read("*all");
			prevFileJson = json.parse(prevFileTxt);
			parentPath = prevFileJson.parent.path;
			parentName = prevFileJson.parent.name;

			texturegroupname = prevFileJson.textureGroupId.name;
			texturegrouppath = prevFileJson.textureGroupId.path;
		end

		local_assert(dirPath:len() > 10, "Fatal error, dirPath was too short, aborting to avoid wiping your computer")
		...
		if false and spriteFile then
			...
			originalSpriteContent = FormatTable(emptyGMSyy,{
				...
				texturegroupname = texturegroupname,
				texturegrouppath = texturegrouppath,
			}
			)
		end
